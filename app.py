...
st.title(":robot_face: Streamlit Sales Dashboard")
st.markdown("_Prototype v0.5.1_")
...
uploaded_file = st.file_uploader(
    label = "NOTE: File upload is disabled. A file named 'Financial Data Clean.xlsx' has been pre-loaded for convenience.",
    disabled=True
)

#######################################
# TEMPORARY HACK FOR WEB DEPLOYMENT
#######################################

if uploaded_file is None:
    uploaded_file = 'data/Financial Data Clean.xlsx'
...
if uploaded_file is None:
    st.info(" Upload a file using the sidebar", icon="ℹ️")
    st.stop()

df = load_data(uploaded_file)
# df = load_data('data/Financial Data Clean.xlsx')
# st.dataframe(df)
...
def plot_metric(label, value, prefix="", suffix="", show_graph=False, color_graph="", title_color="white"):
    fig = go.Figure()

    title_size = 16 if 'Total' in label else 24
    ...
    fig.add_trace(
        go.Indicator(
            ...
            number={
                "prefix": prefix,
                "suffix": suffix,
                "font": {"size": 28, "color": title_color},
            },
            title={
                "text": label,
                "font": {"size": title_size, "color": title_color},
            },
        )
    )
...
with top_left_column:
    ...
    with column_1:
        plot_metric(
            "<b>Total Accounts<br>Receivable", # <br> for newline
            6621280,
            prefix="$",
            suffix="",
            show_graph=True,
